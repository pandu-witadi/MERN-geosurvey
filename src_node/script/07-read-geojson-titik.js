...
//
const create_project = (obj, parent) => {

    let tmp = {
        name: obj['properties']['NAMA_KEGIATAN'] || "",
        parent: parent,
        'type': obj['type'] || "",
    ...
    try {
        ...
        for (let i=0; i < src.length; i++) {
            ...
            if (!!src[i]['properties'][col_name]) {
                const result = await obj.insertOne( create_project(src[i], parent) )
                // console.log(src[i])
...
insert_obj(client, 'MERN-geosurvey-01', 'GeoSpatial', features, target_col, otherKeys).catch(console.dir)
