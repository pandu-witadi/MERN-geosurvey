...
const total_budget_vs_actual_cost = (list) => {
    ...
    for (let i=0; i<list.length; i++) {
        ...
        // console.log(a, b.AFE)
        if ( a.localeCompare("Seismik 2D") == 0 ) {
            obj["Seismik 2D"]["Actual Cost"] += b.AFE.REALISASI_BIAYA_AFE
            if (b.info.TIPE_KONTRAK === 'PSC') {
                obj["Seismik 2D"]["Budget Cost"] += b.AFE.NILAI_AFE
            } else if (b.info.TIPE_KONTRAK === 'GS') {
                obj["Seismik 2D"]["Budget Cost"] += b.info.NILAI_INVESTASI
            }
        } else if ( a.localeCompare("Seismik 3D") == 0 ) {
            obj["Seismik 3D"]["Actual Cost"] += b.AFE.REALISASI_BIAYA_AFE
            if (b.info.TIPE_KONTRAK === 'PSC') {
                obj["Seismik 3D"]["Budget Cost"] += b.AFE.NILAI_AFE
            } else if (b.info.TIPE_KONTRAK === 'GS') {
                obj["Seismik 3D"]["Budget Cost"] += b.info.NILAI_INVESTASI
            }
        } else {
            obj["Survey Lainnya"]["Actual Cost"] += b.AFE.REALISASI_BIAYA_AFE
            if (b.info.TIPE_KONTRAK === 'PSC') {
                obj["Survey Lainnya"]["Budget Cost"] += b.AFE.NILAI_AFE
            } else if (b.info.TIPE_KONTRAK === 'GS') {
...
const trend_budget_cost_vs_actual_cost = (list) => {
    ...
    for (let i=0; i<list.length; i++) {
        ...
        if (project.info.TIPE_KONTRAK === 'PSC') {
             budget = project.AFE.NILAI_AFE
             actual = project.AFE.REALISASI_BIAYA_AFE
...
const create_dashboard = (list_project, list_KKKS, list_WK, num_top_watch) => {
    ...
    for (let i=0; i<list_project.length; i++) {
        ...
        if (project.info.TIPE_KONTRAK === 'PSC') {
             project.calc.TOTAL_INVESTASI = project.AFE.NILAI_AFE
        } else if (project.info.TIPE_KONTRAK === 'GS') {
...
const info_select_project = async (req, res) => {
    ...
    let list_KKKS = await  KKKS.find().sort({}).select({})
    let list_WK = await  WK.find().sort({}).select({})
    ...
    let list_project  = await Project.find(criteria)
